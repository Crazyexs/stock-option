...
def pin_risk_score(S, chain_df, bandwidth_pct=0.03):
    ...
    be 'pinned' near expiry due to large open-interest delta-hedging flows.

    Uses a Gaussian-weighted OI kernel centred on each strike:
        weight(K) = OI(K) × exp(−½ × ((S−K)/(S×bw))²)
    Strikes close to current price get exponentially higher weight.
    High-weight strikes attract pinning; crossing them triggers dealer unwind.
...
def gex_signal(symbol: str, S: float, r: float, hv: float, q: float = 0.0) -> dict | None:
    ...
    Convention (retail-flow): dealers are net short options.
      Call GEX = OI × Gamma × 100 × S  (positive — dealers buy on dips)
      Put  GEX = OI × Gamma × 100 × S  (negative — dealers sell on rallies)
      Net GEX  = Call GEX total − Put GEX total

    Positive net GEX → dealers long gamma → suppress volatility (pin / mean-revert)
    Negative net GEX → dealers short gamma → amplify moves (trending / breakout)
...
def smart_exit_levels(S: float, K: float, T: float, r: float, sigma: float,
                      option_type: str, premium_paid: float, gex: dict) -> dict:
    """
    ...
    ─ Black & Scholes (1973): residual option value at each GEX spot level computed
      with sticky-delta IV adjustment: σ'(S') = σ × (K/S')^0.30  (power-law skew
      slope, Gatheral 2006 calibration for single stocks).
    ─ First-passage time (Inverse Gaussian, drift-free GBM approximation):
      E[T_hit(L)] ≈ |L − S| / (S × σ × √(1/252))  [trading days]
    ...
    def _days(target_S: float) -> float:
        """Drift-free first-passage time in trading days."""
        if target_S <= 0 or daily_1sigma <= 0:
            return np.nan
        return abs(target_S - S) / daily_1sigma
    ...
    def _val(target_S: float, days_elapsed: float) -> float:
        ...
        skew_adj  = float(np.clip((K / target_S) ** 0.30, 0.60, 2.00))
...
def find_trade(symbol=None, S=None, hv=None, q=None):
    ...
    if action == 'b' and gex:
        ...
        exits = smart_exit_levels(
            ...
            option_type=opt_type_str, premium_paid=float(best['Mid Price ($)']), gex=gex
        )
...
def backtest_model():
    ...
    for ei in entry_indices:
        ...
        for pct in STRIKE_PCTS:
            K     = round(S * pct, 2)
            price = bs_price(S, K, T_entry, r, hv, opt_type)
            if np.isnan(price) or price <= 0.01:
                continue
            g     = bs_greeks(S, K, T_entry, r, hv, opt_type)
            delta = g['delta']
            p_itm = prob_expire_itm(S, K, T_entry, r, hv, opt_type)
            if np.isnan(p_itm):
                continue
        ...
        for j in range(ei + 1, expiry_i + 1):
            ...
            curr_px = bs_price(S_j, best_K, T_j, r, hv, opt_type)
            if np.isnan(curr_px):
                continue
        ...
        pnl = ((exit_px - best_entry_px) if action == 'b'
               else (best_entry_px - exit_px)) * 100
        equity += pnl
...
def backtest_scanner(watchlist=None, lookback_days=252, holding_days=14,
    ...
    for scan_idx in scan_indices:
        ...
        for pick in picks:
            ...
            prem      = bs_price(S0, S0, T0, r, sig0, direction)
            ...
            for day_i, (date_i, S_i) in enumerate(px_fwd.items()):
                ...
                # Approximate sticky-strike vol surface tilt
                dist  = abs(float(S_i) - S0) / S0
                sig_i = sig0 * (1 + 0.20 * dist)
                val_i = bs_price(float(S_i), S0, T_rem, r, sig_i, direction)
            ...
            pnl_total = (exit_val - entry_val) * 100 * n_contracts
            acct     += pnl_total
